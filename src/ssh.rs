...
pub struct MockSshSession {
    ...
    /// Number of remaining exec calls that will return SshConnect before succeeding normally.
    pub fail_next_n: AtomicUsize,
}
...
impl MockSshSession {
    ...
    /// Make the next `n` exec calls return a `ToolError::SshConnect` (simulating a dead channel).
